...
def recognizeFaces():
    ...
    # Loop, trying different facial recognition methods
    choice = -1
    while choice != 0:

        # Print menu
        while choice < 0 or choice > 3:
            print('Enter choice of facial recognition method:')
            print('1: Use EigenFaces method')
            print('2: Use FisherFaces method')
            print('3: Use Local Binary Pattern Histogram method')
            print('0: quit')
            schoice = input('')
            choice = int(schoice)
        ...
        while cv2.waitKey(1) == -1:
            success, frame = camera.read()
            if success:
                faces = face_cascade.detectMultiScale(frame, 1.3, 5, minSize=(120,120))
